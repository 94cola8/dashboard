...
/**
 * @description: 获取收取的发票总金额、待处理发票的总金额、发票总数、顾客总数。
 * @author: cola
 * @date: 2024--08--29 16--34
 */
export async function fetchCardData() {
  try {
    ...
    const customerCountPromise = sql`SELECT COUNT(*)
                                     FROM customers`;
    const invoiceStatusPromise = sql`SELECT SUM(CASE
